...
getAttribute a (Element _ as _)
  = maybe "" id (lookup a as)
getAttribute _ _
  = ""

getChildren :: String -> XML -> [XML]
getChildren n (Element _ _ es)
  = filter (matchingChild n) es
getChildren _ _
  = []

matchingChild :: String -> XML -> Bool
matchingChild n (Element n' _ _)
  = n == n'
matchingChild _ _
  = False
...
addChild e (Element n as es)
  = Element n as (es ++ [e])

getValue :: XML -> XML
getValue e
  = case e of
      Element _ _ es -> Text $ concatMap (\(Text s) -> s) (map getValue es)
      Null           -> Text ""
      _              -> e
...
parseAttributes xml 
  | c == '>'  = ([], cs)
  | otherwise = ((name, value) : as, excess'')
  where
    xmlTrim@(c : cs) = skipSpace xml
    (name, excess)   = parseName xmlTrim
    (value, excess') = span (/= '\"') $ tail $ dropWhile (/= '\"') excess
    (as, excess'')   = parseAttributes $ tail excess'
...
-- Pre: There is at least one element on the stack.
parse' "" (Element _ _ (e : _) : _)
  = e
parse' s stack
  | endTag    = parse' (tail $ dropWhile (/= '>') s) (popAndAdd stack)
  | startTag  = parse' xmlRem' ((Element n as []) : stack)
  | otherwise = parse' xmlRem'' (addText text stack)
  where
    startTag         = head s == '<'
    endTag           = startTag && s !! 1 == '/'
    (n, xmlRem)      = parseName $ tail s
    (as, xmlRem')    = parseAttributes xmlRem
    (text, xmlRem'') = span (/= '<') s
...
expandXSL' context xsl@(Element n as es)
  | n == "value-of" = if null textAtPath then [Text ""] else [head textAtPath]
  | n == "for-each" = concat [concatMap (expandXSL' e) es | e <- context']
  | otherwise       = [Element n as (concatMap (expandXSL' context) es)]
  where
    path = split $ getAttribute "select" xsl
    context' = changeContext path context
    attr = concat $ dropWhile ((/= '@') . head) path
    textAtPath
      | null attr = map getValue context'
      | otherwise = map Text (map (getAttribute $ tail attr) context')
expandXSL' _ xsl
  = [xsl]

changeContext :: [String] -> Context -> [XML]
changeContext [] xml
  = [xml]
changeContext (str1 : (c : cs) : strs) xml
  | c == '@'  = getChildren str1 xml
changeContext (str@(c : cs) : strs) xml
  | c == '@'  = [xml]
  | otherwise = concatMap (changeContext strs) (getChildren str xml)
...
split s
  = case s of
      "."       -> []
      ""        -> []
      ('.' : _) -> split remainingPath
      _         -> path : split remainingPath
  where
    (path, remaining) = span (/= '/') s
    remainingPath = drop 1 remaining
